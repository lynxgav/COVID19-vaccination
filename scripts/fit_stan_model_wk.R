...
numberofdays <- 322#dim(hospitalisations_all)[1]
hospitalisations_selected <- hospitalisations_all[1:numberofdays,]

## aggregate to incidence per week
hospitalisations_selected_wk <- 
  hospitalisations_selected[seq(1,numberofdays,7),] +
  hospitalisations_selected[seq(2,numberofdays,7),] +
  hospitalisations_selected[seq(3,numberofdays,7),] +
  hospitalisations_selected[seq(4,numberofdays,7),] + 
  hospitalisations_selected[seq(5,numberofdays,7),] +
  hospitalisations_selected[seq(6,numberofdays,7),] +
  hospitalisations_selected[seq(7,numberofdays,7),]

numberofdays_wk = numberofdays / 7


#demography <- c(495120, 533304, 1068053, 1322534, 1541305, 1416935, 1192854, 973397, 782899, 544818)
...
demography <- pull(read_excel("data/DEMOGRAPHY PT 10.xlsx", skip = 0, sheet = "Population", 
                              col_names = FALSE, col_types = "numeric"), var = 1) # Checked January 2020
sum(demography)
...
contactmatrix_unperturbed <- as.matrix(read.csv("data/CONTACT MATRIX PT BEFORE 10 V.csv", sep = "", header = FALSE))
contactmatrix_distancing <- as.matrix(read.csv("data/CONTACT MATRIX PT AFTER 10 V.csv",sep = "", header = FALSE))
contactmatrix_schools <- as.matrix(read.csv("data/SCHOOL CONTACT MATRIX PT BEFORE 10 V.csv",sep = "", header = FALSE))
...
sero_classes <- c(1, 1, 2, 3, 3, 4, 4, 5, 5, 5)
...
demo_sero_eligible[1] <- (demo_sero_eligible[1] - 86579) ## FIXME!!!! substract population size of 0-1 y.o.
...
datalist <- list(
  ...
  hospitalisations = hospitalisations_selected_wk, 
  Ahosp = 10,
...
stan_model_file <- "scripts/corona_erlangEI_od_wk_model.stan"
...
# print fit
print(fit.rstan, pars=c("epsilon", "zeta", "nu_scale", "nu_simplex", "nu_short" , "inoculum", "beta_short", "r", 
                        "x0", "k0", "x1", "k1", "x2", "k2", "x3", "k3", "x4", "k4", "u1", "u2", "u3", "u4"), digits = 4)

# traces (including warmup)
rstan::traceplot(fit.rstan, pars=c("epsilon", "zeta", "nu_scale", "nu_simplex", "inoculum", "beta_short", "r", 
...
save(fit.rstan, file = "output/wkU4_betasd05_long_alpha_gamma.rda")
#rm(fit.rstan)
#load("output/wkU4.rda")
...
output = as.data.frame(fit.rstan, pars=c("epsilon", "zeta", "nu_short", "inoculum", "beta_short", "r",  
                                         "x0", "k0", "x1", "k1", "x2", "k2", "x3", "k3", "x4", "k4", "u1", "u2", "u3", "u4")) 
write.csv(output, file = "output/wkU4_betasd05_long_alpha_gamma.csv", row.names = FALSE)
...
yHosp_sim025 <- matrix(NA, nrow = numberofdays_wk, ncol = numberofageclasses)
yHosp_sim50 <- matrix(NA, nrow = numberofdays_wk, ncol = numberofageclasses)
yHosp_sim975 <- matrix(NA, nrow = numberofdays_wk, ncol = numberofageclasses)
for (t in 1 : numberofdays_wk) {
...
yHosp_hat025 <- matrix(NA, nrow = numberofdays_wk, ncol = numberofageclasses)
yHosp_hat50 <- matrix(NA, nrow = numberofdays_wk, ncol = numberofageclasses)
yHosp_hat975 <- matrix(NA, nrow = numberofdays_wk, ncol = numberofageclasses)
for (t in 1 : numberofdays_wk) {
  for (c in 1 : numberofageclasses) {
...
for (j in 1:numberofageclasses) {
  dfHosp[[j]] = data.frame(list(ts = ts,
                                ...
                                ## data
                                hospitalizations = hospitalisations_selected_wk[,j]
  ))
}
...
dfHosp[[numberofageclasses+1]] = data.frame(list(ts = ts, 
                                                 ...
                                                 ## data
                                                 hospitalizations = hospitalisations_selected_wk[,1]+hospitalisations_selected_wk[,2]+
                                                   hospitalisations_selected_wk[,3]
))
...
make_logistic_plot(params$k0, params$x0, 0, 210, 23)

## logistic function relaxation
make_logistic_plot(params$k1, params$x1, 0, 210, 60) ## FIXME choose good values

make_logistic_plot(params$k1, params$x2, 0, 210, 178)
