...
# This includes first 5 days with 0 hospitalizations
#hospitalisations <- read.csv("data/hospitalisations PT.csv")
#daystofirstcase <- which(hospitalisations$datum_zhopname == "2020/03/02")
#daysunperturbed <- which(hospitalisations$datum_zhopname == "2020/03/18") 
#daystoend <- 187#30 August #which(hospitalisations$datum_zhopname == "2020/05/30") 
#daystoend <- 254

#date("2020/03/15") - date("2020/03/02")

hospitalisations_all <- read.csv("data/hospitalisations_selected PT 15 Jan 2021 3.csv")
numberofdays <- dim(hospitalisations_all)[1]#300
hospitalisations_selected <- hospitalisations_all[1:numberofdays,]

#hospitalisations_all <- read.csv("data/hospitalisations_selected PT updated.csv")
#numberofdays <- 294#daystoend #dim(hospitalisations_selected)[1]
#hospitalisations_selected <- hospitalisations_all[1:numberofdays, ]
...
SeroData <- read_tsv("data/sero_data PT.tsv")
...
#nu_short_scale_guess <- 0.0429
#nu_short_simplex_guess <- c(0.0145, 0.0182, 0.0271, 0.0272, 0.0431, 0.0479, 0.0774, 0.0799, 0.1651, 0.4997) ## 8 classes
...
beta_short_raw_guess <- c(0.2468, 0.5989) ## 3 age classes (one reference class)
...
stan_model_file <- "scripts/tempU.stan"
...
# print fit
print(fit.rstan, pars=c("epsilon", "zeta", "nu_scale", "nu_simplex", "nu_short" , "inoculum", "p_short", "beta_short", "r", 
...
save(fit.rstan, file = "output/tempU4.rda")
#rm(fit.rstan)
#load("output/temp.rda")
...
output = as.data.frame(fit.rstan, pars=c("epsilon", "zeta", "nu_short", "inoculum", "p_short", "beta_short", "r",  
                                         "x0", "k0", "x1", "k1", "x2", "k2", "x3", "k3", "x4", "k4", "u1", "u2", "u3", "u4")) 
write.csv(output, file = "output/tempU4.csv", row.names = FALSE)

# select sample closest to MAP
hist(params$lp__, nclass = 30)
max(params$lp__)
posmax <- as.numeric(which(params$lp__ == max(params$lp__)))
MAP <- as.data.frame(fit.rstan)[posmax, c("inoculum", "epsilon")] 
MAP
...
make_logistic_plot(params$k0, params$x0, 0, 50, 23)

## logistic function relaxation
make_logistic_plot(params$k1, params$x1, 0, 100, 60) ## FIXME choose good values
