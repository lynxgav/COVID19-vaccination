...
stan_model_file <- "scripts/corona_erlangEI_od_full.stan"
...
save(fit.rstan, file = "output/fullU4.rda")
#rm(fit.rstan)
#load("output/fullU4.rda")
...
output = as.data.frame(fit.rstan, pars=c("epsilon", "zeta", "nu_short", "inoculum", "beta_short", "r", "alpha", "gamma",  
                                         "x0", "k0", "x1", "k1", "x2", "k2", "x3", "k3", "x4", "k4", "u1", "u2", "u3", "u4")) 
write.csv(output, file = "output/fullU4.csv", row.names = FALSE)
...
make_logistic_plot(params$k0, params$x0, 0, 350, 0)

## logistic function relaxation
make_logistic_plot(params$k1, params$x1, 0, 350, 0) ## FIXME choose good values
