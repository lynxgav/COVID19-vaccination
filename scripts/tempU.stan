...
functions {
  ...
  vector corona_model(real t, vector y, real epsilon, real alpha, real gamma, real zeta,  
    ...
    matrix[A,A] C = Cunp * (1-q0) // pre-lockdown
                  + q0 * zeta * Cdis * (1-q1) // lockdown
                  + q1 * (u1 * Cunp + (1-u1) * zeta * Cdis) * (1-q2) // relaxed lockdown
                  + q2 * (u2 * Cunp + (1-u2) * zeta * Cdis) * (1-q3) // 2nd wave
                  + q3 * (u3 * Cunp + (1-u3) * zeta * Cdis) * (1-q4) // 2nd lockdown
                  + q4 * (u4 * Cunp + (1-u4) * zeta * Cdis) ; // 2nd relaxed lockdown
